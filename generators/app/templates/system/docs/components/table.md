  ...
  <<%= sysAbb %>-table
    ...
    <<%= sysAbb %>-table-column
      ...
      <template slot-scope="scope">
        <<%= sysAbb %>-button @click="handleClick" type="text" size="small">Detail</<%= sysAbb %>-button>
        <<%= sysAbb %>-button type="text" size="small">Edit</<%= sysAbb %>-button>
      </template>
    </<%= sysAbb %>-table-column>
  </<%= sysAbb %>-table>
</template>
...
<template>
  <<%= sysAbb %>-table
    ...
    <<%= sysAbb %>-table-column label="Delivery Info">
      <<%= sysAbb %>-table-column
        prop="name"
        label="Name"
        width="120">
      </<%= sysAbb %>-table-column>
      <<%= sysAbb %>-table-column label="Address Info">
        <<%= sysAbb %>-table-column
          prop="state"
          label="State"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <<%= sysAbb %>-button @click="setCurrent(tableData[1])">Select second row</<%= sysAbb %>-button>
    <<%= sysAbb %>-button @click="setCurrent()">Clear selection</<%= sysAbb %>-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <<%= sysAbb %>-button @click="toggleSelection([tableData3[1], tableData3[2]])">Toggle selection status of second and third rows</<%= sysAbb %>-button>
    <<%= sysAbb %>-button @click="toggleSelection()">Clear selection</<%= sysAbb %>-button>
  </div>
</template>
...
```html
<template>
  <<%= sysAbb %>-button @click="resetDateFilter">reset date filter</<%= sysAbb %>-button>
  <<%= sysAbb %>-button @click="clearFilter">reset all filters</<%= sysAbb %>-button>
  <<%= sysAbb %>-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
    ...
    <<%= sysAbb %>-table-column
      ...
      <template slot-scope="scope">
        <<%= sysAbb %>-tag
          ...
          disable-transitions>{{scope.row.tag}}</<%= sysAbb %>-tag>
      </template>
    </<%= sysAbb %>-table-column>
  </<%= sysAbb %>-table>
</template>
...
<template>
  <<%= sysAbb %>-table
    ...
    <<%= sysAbb %>-table-column
      ...
      width="180">
      <template slot-scope="scope">
        <<%= sysAbb %>-popover trigger="hover" placement="top">
          <p>Name: {{ scope.row.name }}</p>
          <p>Addr: {{ scope.row.address }}</p>
          <div slot="reference" class="name-wrapper">
            <<%= sysAbb %>-tag size="medium">{{ scope.row.name }}</<%= sysAbb %>-tag>
          </div>
        </<%= sysAbb %>-popover>
      </template>
    </<%= sysAbb %>-table-column>
    <<%= sysAbb %>-table-column
      ...
      <template slot-scope="scope">
        <<%= sysAbb %>-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Edit</<%= sysAbb %>-button>
        <<%= sysAbb %>-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</<%= sysAbb %>-button>
      </template>
    </<%= sysAbb %>-table-column>
  </<%= sysAbb %>-table>
</template>
...
<template>
  <<%= sysAbb %>-table
    :data="tableData3"
    style="width: 100%">
    <<%= sysAbb %>-table-column type="expand">
...
:::demo To customize row indices, use `index` attribute on <<%= sysAbb %>-table-column> with `type=index`. If it is assigned to a number, all indices will have an offset of that number. It also accepts a method with each index (starting from `0`) as parameter, and the returned value will be displayed as index.
