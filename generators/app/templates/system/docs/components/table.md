  ...
  <sm-table
    ...
    <sm-table-column
      ...
      <template slot-scope="scope">
        <sm-button @click="handleClick" type="text" size="small">Detail</sm-button>
        <sm-button type="text" size="small">Edit</sm-button>
      </template>
    </sm-table-column>
  </sm-table>
</template>
...
<template>
  <sm-table
    ...
    <sm-table-column label="Delivery Info">
      <sm-table-column
        prop="name"
        label="Name"
        width="120">
      </sm-table-column>
      <sm-table-column label="Address Info">
        <sm-table-column
          prop="state"
          label="State"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <sm-button @click="setCurrent(tableData[1])">Select second row</sm-button>
    <sm-button @click="setCurrent()">Clear selection</sm-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <sm-button @click="toggleSelection([tableData3[1], tableData3[2]])">Toggle selection status of second and third rows</sm-button>
    <sm-button @click="toggleSelection()">Clear selection</sm-button>
  </div>
</template>
...
```html
<template>
  <sm-button @click="resetDateFilter">reset date filter</sm-button>
  <sm-button @click="clearFilter">reset all filters</sm-button>
  <sm-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
    ...
    <sm-table-column
      ...
      <template slot-scope="scope">
        <sm-tag
          ...
          disable-transitions>{{scope.row.tag}}</sm-tag>
      </template>
    </sm-table-column>
  </sm-table>
</template>
  ...
  <sm-table
      ...
      <template slot-scope="scope">
        <sm-popover trigger="hover" placement="top">
          ...
          <div slot="reference" class="name-wrapper">
            <sm-tag size="medium">{{ scope.row.name }}</sm-tag>
          </div>
        </sm-popover>
      </template>
    </sm-table-column>
    <sm-table-column
      ...
      <template slot-scope="scope">
        <sm-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Edit</sm-button>
        <sm-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Delete</sm-button>
      </template>
    </sm-table-column>
  </sm-table>
</template>
...
:::demo To customize row indices, use `index` attribute on <sm-table-column> with `type=index`. If it is assigned to a number, all indices will have an offset of that number. It also accepts a method with each index (starting from `0`) as parameter, and the returned value will be displayed as index.
